...
func (m *PostgresDBRepo) AllMovies() ([]*models.Movie, error) {
	...
	// If query successful, cancel at the end
	defer cancel()

	// Create query *Becareful with sql injection*
	query := `
		SELECT
...
func (m *PostgresDBRepo) GetUserByEmail(email string) (*models.User, error) {
	...
	// Create timeout to connect db
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()

	// Create query to find user in users table
	// Use $1 to aviod SQL injection (Parameterized)
	query := `
...
// Create function to find user by ID in order to find if user has already logged in or not
...
func (m *PostgresDBRepo) AllGenres() ([]*models.Genre, error) {
	...
	query := `SELECT id, genre, created_at, updated_at 
				FROM genres
				ORDER BY genre`
	rows, err := m.DB.QueryContext(ctx, query)
	if err != nil {
...
func (m *PostgresDBRepo) UpdateMovieGenres(id int, genresID []int) error {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()
